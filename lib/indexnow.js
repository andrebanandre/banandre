...
const keyLocationNoWww = "https://banandre.com/26ecf4563d764768a1293f4edac8c4a2.txt";
...
async function main() {
  try {
    ...
    if (filteredUrls.length > 0) {
      ...
      // First try with www
      console.log("\n📡 Testing with www.banandre.com...");
      try {
        const response = await postToIndexNow(filteredUrls, false);
        console.log("✅ IndexNow API Response (with www):");
        console.log("Status:", response.statusCode, response.statusMessage);
        console.log("Data:", response.data);

        // If first attempt fails, try without www
        if (response.statusCode !== 200) {
          console.log("\n🔄 First attempt failed, trying without www...");
          const responseNoWww = await postToIndexNow(filteredUrls, true);
          console.log("✅ IndexNow API Response (without www):");
          console.log("Status:", responseNoWww.statusCode, responseNoWww.statusMessage);
          console.log("Data:", responseNoWww.data);
        }
      } catch (error) {
